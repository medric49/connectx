...
import utils
from utils import check_winning_board, check_losing_board


def get_available_actions(state):
    board, mark, player = state
    grid = utils.make_grid(board)

    if mark != player:
        return []

    if utils.finish_board(board):
        return []
...
def get_reward(next_state):
    ...
    if check_winning_board(next_board, 3 - player):
        reward += -100
    if utils.check_almost_winning_board(next_board, player):
        reward += 10
    if utils.check_almost_winning_board(next_board, 3 - player):
        reward += -25
    if utils.check_defending_board(next_board, player):
        reward += 15
        ...
        while True:
            ...
            for state in states:
                ...
                if PI[state] is not None:
                    next_state, reward = get_next_state_and_reward(state, PI[state])
                    V[state] = reward + gamma * V[next_state]
                elif PI[opp_state] is not None:
                    opp_next_state, opp_reward = get_next_state_and_reward(opp_state, PI[opp_state])
